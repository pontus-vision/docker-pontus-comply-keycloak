...
if [[ (! -d /mnt/keycloak/update-keycloak-2021-07-11T1527)  || ("true" = "${PV_FORCE_REINSTALL}" ) ]]; then 
  ...
  mkdir     /mnt/keycloak/update-keycloak-2021-07-11T1527
...
#   /mnt/keycloak/keycloak-14.0.0/bin/add-user-keycloak.sh -r master -u admin  -p ${PV_KEYCLOAK_ADMIN_PASSWD:-Adminpa55word2@}
 # /mnt/keycloak/keycloak-14.0.0/bin/add-user-keycloak.sh -r master -u formiti_administrator  -p ${PV_KEYCLOAK_ADMIN_PASSWD:-Adminpa55word@2£}
 # /mnt/keycloak/keycloak-14.0.0/bin/add-user-keycloak.sh -r pontus -u formiti_administrator  -p ${PV_KEYCLOAK_ADMIN_PASSWD:-Adminpa55word@2£}
...
#   /mnt/keycloak/keycloak-14.0.0/bin/add-user-keycloak.sh -r pontus -u leo  -p ${PV_KEYCLOAK_ADMIN_PASSWD:-Adminpa55word2@}
  ...
  -Dkeycloak.frontendUrl=${PV_KEYCLOAK_FRONTEND_URL:-"https://test1.formiti-demo.pontusvision.com/auth"}
  exit 0
fi
